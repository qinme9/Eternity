...
public class EternalLingeringPotionItem extends LingeringPotionItem {

    public EternalLingeringPotionItem(Properties properties) {
        super(properties);
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        level.playSound(null, player.getX(), player.getY(), player.getZ(),
                SoundEvents.LINGERING_POTION_THROW, SoundSource.PLAYERS, 0.5F,
                0.4F / (level.getRandom().nextFloat() * 0.4F + 0.8F));
        if (!level.isClientSide) {
            // 1.21.1 uses is(Items.LINGERING_POTION) not instanceof, so we need a vanilla item stack
            ItemStack fakeStack = new ItemStack(Items.LINGERING_POTION);
